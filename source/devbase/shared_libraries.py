...
###     result, output, errors = devenv.process.execute3(command)
###     assert not output, output
###     assert not errors, errors
### 
###     # Not sure how to test result of depends.exe for success and failure.
###     # For now, assume that if something goes wrong, we'll notice it later on.
### 
...
###     with open(walker_result_filename, "rb") as csv_file:
...
###                     (shared_library_path_name.find("python27.dll") ==
###                     -1) and (not devenv.path_names.path_names_are_equal(
...
def determine_shared_library_dependencies(
    ...
    assert devenv.filesystem.file_is_dll_client(path_name), path_name
...
###         devenv.path_names.objects_directory_path_name() if "OBJECTS" in \
###             os.environ else None
###     pcrteam_extern_directory_path_name = os.path.realpath(
###         devenv.path_names.pcrteam_extern_directory_path_name())
### 
###     for shared_library_path_name in shared_library_path_names:
###         directory_path_name = os.path.dirname(shared_library_path_name)
### 
###         if objects_directory_path_name and \
###                 devenv.path_names.path_names_are_equal(
###                     devenv.path_names.commonprefix([objects_directory_path_name,
###                         directory_path_name]),
###                 objects_directory_path_name):
###             package_shared_library_path_names.append(shared_library_path_name)
###         elif devenv.path_names.path_names_are_equal(
###                 devenv.path_names.commonprefix(
...
###         devenv.shared_library_dependencies([os.path.join(project_prefix,
###             path_name)])
...
###         devenv.path_names.path_names_are_equal(
###             devenv.path_names.commonprefix([dll_file_name, project_prefix]),
###             project_prefix)]
### 
### 
...
###     offset = devenv.path_name_components(offset)
